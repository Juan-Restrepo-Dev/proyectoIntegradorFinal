...
from apps.generator.templates_prompt import (
    format_system_prompt_strategy,
    format_system_prompt_image_description,
    format_sytem_prompt_text_post,
)
from apps.generator.schema import SocialPostRequest
from shared.services.ai.google.image import generate_ad_content
from shared.services.ai.google.system_instructions import (
    generate_inference_instructions,
)
...
json_schema_stategy = {
    ...
    "properties": {
        ...
        "analisis_publico_objetivo": {
            "type": "OBJECT",
            "properties": {
                "perfil_demografico": {"type": "STRING"},
        ...
        "sugerencias_segmentacion": {
            ...
            "properties": {
                ...
                "datos_demograficos": {"type": "STRING"},
                "intereses_y_comportamientos": {"type": "STRING"},
            },
        },
...
async def generate_post_strategy(data_strategy):
    if isinstance(data_strategy, dict):
        ...
        data_dict = (
            data_strategy.model_dump()
            if hasattr(data_strategy, "model_dump")
            else data_strategy.dict()
        )
    ...
    for part in strategy:
        if part.text is not None:
            print("--- Estrategia Generado por la IA ---")
...
async def generate_post_text(data_post_text):
    if isinstance(data_post_text, dict):
        ...
        data_dict = (
            data_post_text.model_dump()
            if hasattr(data_post_text, "model_dump")
            else data_post_text.dict()
        )
    ...
    for part in post_text:
        if part.text is not None:
            ...
            print("texto generado:",part.text)
            response = json.loads(part.text)
    return response
...
async def generate_image_description(data_image_description):
    if isinstance(data_image_description, dict):
        ...
        data_dict = (
            data_image_description.model_dump()
            if hasattr(data_image_description, "model_dump")
            else data_image_description.dict()
        )
    ...
    for part in image_description:
        if part.text is not None:
            print("--- Texto Generado por la IA ---")
            print("descripcion de imagen para generar:",part.text)
            response = json.loads(part.text)
            return response
        else:
            return "no se genero descripcion de imagen"
...
async def generate_image(url, PromptImage, company_id, max_retries=3):
    """
    Intenta generar una imagen con reintentos controlados.
    """
    for attempt in range(max_retries):
        print(f"Intento {attempt + 1}/{max_retries} para generar la imagen.")
        try:
            generated_parts = generate_ad_content(
                image_context_url=url, prompt_text=PromptImage, project_id=my_project_id
            )

            for part in generated_parts:
                # 1. Verifica si la imagen está en el formato correcto
                if (
                    part.inline_data
                    and isinstance(part.inline_data.data, (bytes, bytearray))
                    ...
                    image_url = await upload_image_to_storage(
                        image_data, destination_path, part.inline_data.mime_type
                    )
                    return image_url  # 3. Si tiene éxito, retorna y sale del bucle

            print(f"No se generó ninguna imagen válida en el intento {attempt + 1}.")
            # 4. Si el bucle termina sin encontrar imagen, reintenta
            await asyncio.sleep(2)  # Espera 2 segundos antes de reintentar

        except Exception as e:
            print(f"Ocurrió un error en el intento {attempt + 1}: {e}")
            await asyncio.sleep(2)

    print(f"Falló la generación de la imagen después de {max_retries} intentos.")
    return None  # 5. Si todos los reintentos fallan, retorna None


async def generate_social_post(
    basePrompt: SocialPostRequest, user_data: dict = Depends(get_current_user_and_role)
):
    ...
    print(
        "Datos del prompt base:",
        base_prompt_dict["product_data"]["multimedia"]["main_image"],
    )
